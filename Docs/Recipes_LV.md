...
First multiblock structure. Replaces and far outperforms the basic Boiler for dedicated steam production.
...
| Electric Blast Furnace | Brick Furnace | 64W | New; high-temp alloys + Boules; steel 4× faster |
| Centrifuge | — | 32W | New; separates mixed material/fluid outputs |
...
> Replaces Primitive Furnace — faster (2s vs 4s) and automated. Does not handle high-temp alloys; those go in the Electric Blast Furnace.
...
Automated version of Steam Workbench. Inherits all Steam Workbench recipes. New LV recipes:

| Input 1 | Qty | Input 2 | Qty | Output | Qty | Time (s) |
|---|---|---|---|---|---|---|
| Iron Plate | 2 | Copper Wire | 4 | LV Motor | 1 | 8 |
| *(machine crafting recipes TBD)* | | | | | | |

> Unlike the Steam Workbench, the Assembler runs without player interaction — fully automatable via item pipes.
...
| Silicon Dust | 64 | — | — | Silicon Boule | 1 | 60 |
...
> Boule production is slow by design — one Blast Furnace runs a long batch unattended.
...
> Magnetic Steel is required for LV motors and certain circuit components.
...
> 1 Ingot → 2 Rods across all metals. Rods are required for motors, axles, and circuit components.
...
Separates mixed material or fluid outputs that other machines produce as combined byproduct streams.

| Input 1 | Qty | Input 2 | Qty | Output 1 | Qty | Output 2 | Qty | Time (s) |
|---|---|---|---|---|---|---|---|---|
| *(TBD — defined as LV chain byproducts are finalized)* | | | | | | | | |

> Centrifuge use cases: separating ore macerate byproduct streams, processing mixed fluid outputs from Chemical Reactor chains. Recipes added as content is designed.
